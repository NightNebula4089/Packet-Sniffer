...
EthernetPacket::EthernetPacket(const PayloadInfo& payload_info) : Packet(payload_info) {
    const u_char *packet = payload_info.payload;
    size_t size = payload_info.payload_len;
    ...
    this->payload_info = { packet + ethernet_payload_offset, (size_t)ethernet_payload_len }; // Set payload information
...
uint16_t EthernetPacket::getProtocol() const {
    return ether_type;
}

void EthernetPacket::printInfo() const {
    switch(ether_type){
        case 0x0800: // IPv4
...
PayloadInfo EthernetPacket::getPayload() const {
    return this->payload_info;
}

ProtoSpace EthernetPacket::getProtoSpace() const {
    return ProtoSpace::EtherType;
}